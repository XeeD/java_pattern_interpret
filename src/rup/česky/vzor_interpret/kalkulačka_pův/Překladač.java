...
/*******************************************************************************
 * Třída Překladač přestavuje lexikální a syntaktický analyzátor,
 * který převedou text do stromové struktury vnitřní reprezentace programu.
 * <p>
...
//== SOUKROMÉ A POMOCNÉ METODY TŘÍDY ===========================================
    ...
    private static IAritmVýraz přeložSoučet( Zdroj zdroj )
    {
        ...
        if( zdroj.naŘadě == '+' )
            součet = new Součet( součet, přeložSoučin( zdroj ) );
        else if( zdroj.naŘadě == '-' )
            součet = new Rozdíl( součet, přeložSoučin( zdroj ) );
        return součet;
    }
    ...
    private static IAritmVýraz přeložSoučin( Zdroj zdroj )
    {
        ...
        if( zdroj.naŘadě == '*' )
            součin = new Součin( součin, přeložČlen( zdroj ) );
        else if( zdroj.naŘadě == '/' )
            součin = new Podíl( součin, přeložČlen( zdroj ) );
        return součin;
    }
    ...
    private static IAritmVýraz přeložČlen( Zdroj zdroj )
    {
        ...
        if( znak == '-' ) {
            ...
            return new Součin( new Konstanta( -1 ), přeložPoločlen( zdroj ) );
        }else if( znak == '+' ) {
            znak = dalšíZnak( zdroj );
        }
     ...
     * Metoda je ale vyvolávána pouze v reakci na nejmerkantněšjí chyby.
     */
